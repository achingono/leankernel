...
public class HttpRequestToolTests
{
    ...
    [Fact]
    public async Task HttpRequestTool_honors_configured_max_redirects()
    {
        var callCount = 0;
        var handler = new StubHttpMessageHandler(_ =>
        {
        ...
        var tool = HttpRequestTool.Create(CreateScopeFactory(handler, maxRedirects: 0));
    ...
    private static IServiceScopeFactory CreateScopeFactory(HttpMessageHandler handler, int? maxRedirects = null)
    {
