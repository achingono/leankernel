...
namespace LeanKernel.Services.Gateway.Sessions;
    ...
    ILogger<DbAgentStateStore> logger) : AgentSessionStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (entity is not null && !string.IsNullOrWhiteSpace(entity.StateJson))
        {
            try
            {
                ...
                return JsonSerializer.Deserialize<ChatClientAgentSession>(doc, JsonOptions)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var jsonElement = JsonSerializer.SerializeToElement(typedSession, JsonOptions);
