...
public sealed class GBrainService : IMemoryService
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GBrainService"/> class.
        ...
        CancellationToken ct = default)
    {
        ...
        this._logger.LogDebug("GBrain knowledge search: {Query} (max={Max})", query, maxResults);

        try
        {
            var result = await this._client.CallToolAsync("search", new { query, limit = maxResults }, ct)
        ...
        {
            this._logger.LogWarning(ex, "GBrain knowledge search failed for query: {Query}", query);
            return [];
        }
    }
    ...
    public async Task<MemoryPage?> GetPageAsync(string key, CancellationToken ct = default)
    {
        ...
        this._logger.LogDebug("GBrain knowledge get_page: {Key}", key);

        try
        {
            var result = await this._client.CallToolAsync("get_page", new { slug = key }, ct)
                .ConfigureAwait(false);
        ...
        {
            this._logger.LogWarning(ex, "GBrain get_page failed for key: {Key}", key);
            return null;
        }
    }
    ...
    public Task PutPageAsync(string key, string content, CancellationToken ct = default)
    {
        ...
        return this.PutPageCoreAsync(key, content, ct);
    }

    private async Task PutPageCoreAsync(string key, string content, CancellationToken ct)
    {
        this._logger.LogDebug("GBrain knowledge put_page: {Key} ({Length} chars)", key, content.Length);

        await this._client.CallToolAsync("put_page", new { slug = key, content }, ct)
            .ConfigureAwait(false);
    }
