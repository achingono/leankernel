...
public sealed class GBrainMcpClient : IGBrainMcpClient
{
    ...
    {
        var requestId = Interlocked.Increment(ref this._requestId);
        ...
        this._logger.LogDebug("GBrain MCP call: {Tool} (id={Id})", toolName, requestId);

        var result = await this.SendMcpRequestAsync(request, ct).ConfigureAwait(false);

        if (result?.Error is not null)
        {
            this._logger.LogWarning("GBrain MCP error: {Code} {Message}", result.Error.Code, result.Error.Message);
    ...
    {
        using var response = await this._httpClient.PostAsJsonAsync(string.Empty, request, Constants.Serialization.JsonOptions, ct).ConfigureAwait(false);
