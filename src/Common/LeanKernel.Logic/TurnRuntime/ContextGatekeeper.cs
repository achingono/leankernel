        ...
        foreach (var item in systemItems)
        {
                ...
                item.EstimatedTokens <= remainingSystemBudget)
            {
                ...
                context.AdmissionTrace.Add(new AdmissionRecord
                {
                    Source = item.Source,
                    Admitted = true,
                    Reason = Constants.TurnRuntime.AdmissionReason.SystemContext,
        ...
        foreach (var item in retrievalItems)
        {
            if (retrievalCount >= _settings.MaxRetrievalCandidates)
            {
                context.AdmissionTrace.Add(new AdmissionRecord
                {
                    Source = item.Source,
                    Admitted = false,
                    Reason = Constants.TurnRuntime.AdmissionReason.MaxCandidatesReached,
                    TokenCost = item.EstimatedTokens,
                    RemainingBudget = context.RemainingBudget
                });
                continue;
            ...
            if (item.Score < _settings.MinRetrievalScore)
            {
                context.AdmissionTrace.Add(new AdmissionRecord
                {
                    Source = item.Source,
                    Admitted = false,
                    Reason = Constants.TurnRuntime.AdmissionReason.LowScore,
                    TokenCost = item.EstimatedTokens,
                    RemainingBudget = context.RemainingBudget
                });
                continue;
            ...
            else
            {
                context.AdmissionTrace.Add(new AdmissionRecord
                {
                    Source = item.Source,
                    Admitted = false,
                    Reason = Constants.TurnRuntime.AdmissionReason.BudgetExhausted,
                    TokenCost = item.EstimatedTokens,
                    RemainingBudget = context.RemainingBudget
                });
            }
