...
public static class BrowserToolDefinitions
{
    ...
    private static WebwrightSettings GetBrowserConfig(IServiceProvider serviceProvider)
        => serviceProvider.GetService<IOptions<AgentSettings>>()?.Value.Tools.Webwright ?? new WebwrightSettings();
