...
public sealed class WebwrightClient : IWebwrightClient
{
    ...
    public Task<BrowserArtifactContent> GetArtifactAsync(string runId, string artifactId, int maxBytes, CancellationToken ct = default)
    {
        ...
        return GetArtifactCoreAsync(runId, artifactId, maxBytes, ct);
    }

    private async Task<BrowserArtifactContent> GetArtifactCoreAsync(string runId, string artifactId, int maxBytes, CancellationToken ct)
    {
