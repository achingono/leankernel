...
{
    /// <summary>
    /// Creates a tool definition for touching (creating or updating timestamp of) files.
    /// </summary>
    /// <param name="scopeFactory">The service scope factory.</param>
    /// <returns>A <see cref="ToolDefinition"/> for the file touch tool.</returns>
    public static ToolDefinition Create(IServiceScopeFactory scopeFactory)
    {
        ...
        return new ToolDefinition
        {
            ...
            Handler = async (args, ct) =>
            {
                ...
                if (fullPath is null)
                {
                    return new ToolResult { ToolName = "file_touch", Success = false, Error = Constants.FileSystem.Errors.PathOutsideAllowedDirectory };
                }
