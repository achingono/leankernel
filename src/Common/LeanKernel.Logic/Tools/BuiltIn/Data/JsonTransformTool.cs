...
public static class JsonTransformTool
{
    ...
    private static ToolResult Handle(IReadOnlyDictionary<string, object?> args)
    {
        ...
        if (string.IsNullOrWhiteSpace(inputRaw)) return Fail("Input is required");
        if (string.IsNullOrWhiteSpace(operationsRaw)) return Fail("Operations are required");

        JsonNode? current;
        try { current = JsonNode.Parse(inputRaw); }
        catch (JsonException ex) { return Fail($"Invalid input JSON: {ex.Message}"); }

        JsonArray operations;
        try { operations = JsonNode.Parse(operationsRaw) as JsonArray ?? new JsonArray(); }
        catch (JsonException ex) { return Fail($"Invalid operations JSON: {ex.Message}"); }

        if (operations.Count > MaxOperations) return Fail($"Too many operations. Maximum is {MaxOperations}.");

        for (var i = 0; i < operations.Count; i++)
        {
            var op = operations[i] as JsonObject;
            if (op is null) return Fail($"Operation {i + 1} must be a JSON object");
    ...
    {
        error = null;
        if (!TryReadStringProperty(op, "path", out var path)) { error = "Missing 'path'"; updated = null; return false; }
        var segments = path.Split('.', StringSplitOptions.RemoveEmptyEntries);
        var node = current;
        foreach (var segment in segments)
        {
            if (node is JsonObject obj) { node = obj[segment]; }
            else if (node is JsonArray arr && int.TryParse(segment, out var idx)) { node = idx < arr.Count ? arr[idx] : null; }
    ...
    {
        error = null; updated = current;
        if (current is not JsonArray arr) { error = "Input must be an array for project"; return false; }
        if (!op.TryGetPropertyValue("fields", out var fieldsNode) || fieldsNode is not JsonArray fieldsArr) { error = "Missing 'fields' array"; return false; }
        ...
        foreach (var item in arr)
        {
            if (item is JsonObject obj)
            {
                ...
                {
                    if (obj.TryGetPropertyValue(field, out var val)) projected[field] = val?.DeepClone();
                }
    ...
    {
        error = null; updated = current;
        if (current is not JsonArray arr) { error = "Input must be an array for filter_equals"; return false; }
        if (!TryReadStringProperty(op, "field", out var field)) { error = "Missing 'field'"; return false; }
        if (!op.TryGetPropertyValue("value", out var filterValue)) { error = "Missing 'value'"; return false; }
        var result = new JsonArray();
        foreach (var item in arr)
        {
    ...
    {
        error = null; updated = current;
        if (current is not JsonArray arr) { error = "Input must be an array for sort"; return false; }
        if (!TryReadStringProperty(op, "field", out var field)) { error = "Missing 'field'"; return false; }
        ...
        if (descending) sorted = sorted.OrderByDescending(x => x.Value, JsonNodeComparer.Instance).ThenBy(x => x.Index).ToList();
        var result = new JsonArray();
        foreach (var item in sorted) result.Add(item.Item?.DeepClone());
        updated = result;
        return true;
    }
    ...
    {
        error = null; updated = current;
        if (current is not JsonArray arr) { error = "Input must be an array for slice"; return false; }
        ...
        var result = new JsonArray();
        for (var i = start; i < end; i++) result.Add(arr[i]?.DeepClone());
        updated = result;
        return true;
    }
    ...
    {
        error = null; updated = current;
        if (current is not JsonArray arr) { error = "Input must be an array for flatten"; return false; }
    ...
    private static void FlattenArray(JsonArray source, JsonArray target, int depth)
    {
        ...
        {
            if (item is JsonArray nested && depth > 0) FlattenArray(nested, target, depth - 1);
    ...
    {
        value = string.Empty;
        if (!obj.TryGetPropertyValue(name, out var node) || node is not JsonValue val) return false;
        value = val.ToString();
        return true;
    }

    private static bool FailOp(string msg, out string error) { error = msg; return false; }
    ...
    private sealed class JsonNodeComparer : IComparer<JsonNode?>
    {
        ...
        public int Compare(JsonNode? x, JsonNode? y)
        {
            ...
            {
                if (xv.TryGetValue(out double xd) && yv.TryGetValue(out double yd)) return xd.CompareTo(yd);
                if (xv.TryGetValue(out string? xs) && yv.TryGetValue(out string? ys)) return string.Compare(xs, ys, StringComparison.Ordinal);
