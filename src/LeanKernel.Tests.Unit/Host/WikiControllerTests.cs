...
public class WikiControllerTests
{
    ...
    public async Task GetDimensions_ReturnsSummary()
    {
        ...
        var import = Substitute.For<IWikiImportService>();
        ...
        var controller = new WikiController(wiki, migration, import);
    ...
    [Fact]
    public async Task ImportOpenClaw_ReturnsSummary()
    {
        var wiki = Substitute.For<IWikiStore>();
        var migration = Substitute.For<IWikiMigrationService>();
        var import = Substitute.For<IWikiImportService>();
        import.ImportOpenClawAsync(Arg.Any<OpenClawImportRequest>(), Arg.Any<CancellationToken>())
            .Returns(new OpenClawImportResult("run-1", true, WikiExtractionStrategy.Deterministic, 10, 100, 70, 30, 12, "/tmp/audit.json"));

        var controller = new WikiController(wiki, migration, import);
        var result = await controller.ImportOpenClaw(dryRun: true, skipRemoteSync: false, ct: CancellationToken.None);

        var ok = Assert.IsType<OkObjectResult>(result);
        var payload = Assert.IsType<OpenClawImportResult>(ok.Value);
