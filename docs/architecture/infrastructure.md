...
| `gbrain` | Bun / `garrytan/gbrain` | Wiki-backed knowledge service and MCP HTTP endpoint for retrieval and page operations | `8789` | Shared Postgres database plus `gbrain-data` working directory volume |
...
- `gbrain` depends on the shared Postgres service and stores wiki/vector state in the `public` schema with pgvector-backed embeddings.
...
| GBrain | `LEANKERNEL__GBRAIN__BASEURL`, `GBRAIN_PORT`, `GBRAIN_EMBEDDING_MODEL`, `GBRAIN_EMBEDDING_DIMENSIONS`, `LITELLM_BASE_URL`, `LITELLM_API_KEY` | MCP endpoint, embedding model/dimension pairing, and LiteLLM proxy wiring for GBrain's model calls |
...
- **Postgres volume:** LeanKernel application state, diagnostics, schedules, GBrain wiki pages, and GBrain pgvector embeddings live in the persistent Postgres volume.
- **GBrain volume:** the `gbrain-data` Docker volume holds the Bun-hosted GBrain working directory and engine token file.
...
- **Recovery model:** restore Postgres for LeanKernel and GBrain state, restore `gbrain-data` for the GBrain working directory/token state, then restart the Compose stack.
