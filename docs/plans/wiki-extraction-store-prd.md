...
`WikiStore` must maintain generated metadata in `data/wiki/.meta/`:
...
5. Use `(inferredDimension, normalizedSubject)` as the merge key. Cross-dimension subject collisions (e.g. the same slug `assistant` appearing across `who` and `what`) are NOT auto-merged; they go to `data/wiki/.meta/quarantine/{originalRelativePath}` with a `reason` field appended to `migration.json`.
...
9. Write `data/wiki/.meta/migration.completed` with the run timestamp.
...
- AC-5: `data/wiki/.meta/index.json` can be deleted and rebuilt from markdown alone, and the rebuilt index is structurally equal (modulo `builtAt`) to the previous one — verified by an automated test.
...
- Should quarantined wiki files live under `data/wiki/.meta/quarantine` or a visible `data/wiki/quarantine` folder?
...
2. Store index files under `data/wiki/.meta/index.json`.
...
5. Rebuild from markdown files in the six canonical dimension folders. Exclude `.meta`, `llm`, and quarantine folders.
