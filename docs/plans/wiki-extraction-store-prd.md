...
Wiki vector indexing is owned by the existing Python sidecar `config/indexer/indexer.py`, which watches `data/wiki/`, parses markdown frontmatter, embeds, and upserts wiki vectors to `LEANKERNEL_knowledge` and document vectors to `documents`. No C# component writes to Qdrant.
