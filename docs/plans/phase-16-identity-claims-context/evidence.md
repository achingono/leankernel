...
| Profile validation | `src/Services/LeanKernel.Services.Gateway/Programs.cs:89-96` | `ValidateOnStart()` for non-negative bounds and `MaxPromptTokens > 0` |
...
| Appsettings | `src/Services/LeanKernel.Services.Gateway/appsettings.json` | `Identity:ClaimsContext` section with defaults |
