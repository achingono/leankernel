...
| Tenant resolution (host-based) | `src/Services/LeanKernel.Services.Gateway/Providers/TenantResolutionMiddleware.cs`, `IdentityResolver.ResolveTenantAsync` | Web resolves tenant by HTTP host; channels need a channel→tenant scope instead |
...
| Gateway hardcoded channel | `src/Services/LeanKernel.Services.Gateway/Providers/TenantResolutionMiddleware.cs` (`ChannelEntity.OpenAiHttpName`) | Must instead resolve `ChannelId` from channel claims |
...
| Memory client (policy consumer) | `src/Services/LeanKernel.Services.Gateway/Memory/GBrainMemoryClient.cs` | Phase 10 enforces the policy here (`memory/{tenantId}/{...}/{channelId}/{key}`) |
