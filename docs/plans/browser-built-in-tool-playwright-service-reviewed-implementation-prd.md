...
1. Add `BrowserServiceConfig` under `LeanKernel.Abstractions/Configuration`, wire it into `LeanKernelConfig`, `ProviderNames`, `appsettings`, `.env.example`, and Docker Compose.
2. Add `IBrowserServiceClient` under `LeanKernel.Abstractions/Interfaces` and implement the typed client, DTOs, error mapping, validation, and four tools under `LeanKernel.Tools/BuiltIn/Browser`.
3. Register browser tools conditionally in `ToolsServiceCollectionExtensions.AddLeanKernelTools` only when `LeanKernel:BrowserService:Enabled` is true.
4. Register the named browser `HttpClient` and `BrowserServiceHealthProbe` conditionally in `LeanKernelHardeningServiceCollectionExtensions`, using the existing correlation handler chain and authenticated `/ready`.
5. Build the sidecar under `config/browser-service/` with FastAPI, Webwright pinned to `0.0.7`, Playwright Firefox, async queue/concurrency, idempotency metadata, subprocess lifecycle, manifest-gated artifact access, bearer auth, `/health`, and `/ready`.
6. Update Compose so `browser-service` health checks call unauthenticated `/health`, operational calls require the blank-by-default `BROWSER_SERVICE_API_TOKEN`, and Webwright routes model calls through LiteLLM with a dedicated browser key.
...
10. Validate with the repository restore/build/test sequence, coverage, Sonar, Docker Compose build, and browser-service health checks where available.

## Review Corrections Applied
- Use `config/browser-service/` as the sidecar build context to match repository sidecar conventions.
...
- Place `IBrowserServiceClient` in Abstractions so the contract is not hidden in the tool implementation layer.
...
- Browser automation remains disabled by default via `LeanKernel:BrowserService:Enabled=false`.
...
- Compose uses a dedicated `BROWSER_SERVICE_LITELLM_KEY`; local developers may set it to a scoped LiteLLM virtual key.
